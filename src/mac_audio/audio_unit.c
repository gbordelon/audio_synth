...
audio_unit_input_init()
{
  ...
  AudioComponentDescription desc;
  desc.componentType = kAudioUnitType_Output;
  desc.componentSubType = kAudioUnitSubType_HALOutput;//kAudioUnitSubType_VoiceProcessingIO;//kAudioUnitSubType_DefaultOutput;//kAudioUnitSubType_RemoteIO;
  desc.componentFlags = 0;
  desc.componentFlagsMask = 0;
  desc.componentManufacturer = kAudioUnitManufacturer_Apple;
  ...
  // Get audio units
  status = AudioComponentInstanceNew(inputComponent, &audioUnitMicInput);
  checkStatus(status);

  UInt32 flag = 1;
  // Enable input for mic/line-in
  status = AudioUnitSetProperty(audioUnitMicInput, 
                  kAudioOutputUnitProperty_EnableIO, 
                  kAudioUnitScope_Input, 
                  kInputBus,
                  &flag, 
  ...
  // Initialize
  status = AudioUnitInitialize(audioUnitMicInput);
  checkStatus(status);

  // allocate bufferlist for reading mic data
  UInt32 bufferSizeBytes = 512 * sizeof(Float32) * NUM_CHANNELS; // 512 frames
  UInt32 propertySize = offsetof(AudioBufferList, mBuffers[0]) + (sizeof(AudioBuffer) * NUM_CHANNELS);

  _input_buffer = (AudioBufferList *) calloc(1, propertySize);
  _input_buffer->mNumberBuffers = NUM_CHANNELS; // channels per frame
  ...
  return audioUnitMicInput;
}
*/
/*
AudioComponentInstance
audio_unit_output_init()
{
  OSStatus status;
  ...
  // not needed since this is the default setting
  UInt32 flag = 1;
//  // Enable IO for playback
...
//                  &flag, 
//                  sizeof(flag));
//  checkStatus(status); // invalid property
